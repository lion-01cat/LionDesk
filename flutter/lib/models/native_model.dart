...
class PlatformFFI {
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libliondesk.so')
        : isLinux
            ? DynamicLibrary.open('libliondesk.so')
            : isWindows
                ? DynamicLibrary.open('libliondesk.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibliondesk.dylib") :
  ...
  void _startListenEvent(RustdeskImpl liondeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = liondeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
