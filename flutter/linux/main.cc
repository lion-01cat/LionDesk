...
#define RUSTDESK_LIB_PATH "libliondesk.so"
typedef bool (*LionDeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_liondesk_core_main() {
   void* libliondesk = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   if (!libliondesk) {
      fprintf(stderr,"Failed to load \"libliondesk.so\"\n");
      char* error;
   ...
   auto core_main = (LionDeskCoreMain) dlsym(libliondesk,"liondesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"liondesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
