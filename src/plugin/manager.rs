...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "liondesk".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/liondesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "LionDesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "LionDesk wants to uninstall the plugin",
    )
}
