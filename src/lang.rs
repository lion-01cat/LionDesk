...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_liondesk() {
            if s.contains("LionDesk")
                && !name.starts_with("upgrade_liondesk_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("LionDesk") {
                    s = s.replace("LionDesk", &app_name);
                } else {
                    // https://github.com/liondesk/liondesk-server-pro/issues/845
                    // If app_name contains "LionDesk" (e.g., "LionDesk-Admin"), we need to avoid
                    // replacing "LionDesk" within the already-substituted app_name, which would
                    // cause duplication like "LionDesk-Admin" -> "LionDesk-Admin-Admin".
                    //
