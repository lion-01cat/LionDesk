// https://github.com/liondesk/liondesk/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteLionDeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes LionDesk Test certificates and wrong key stores
//
...
BOOL DeleteLionDeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteLionDeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteLionDeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteLionDeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteLionDeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
